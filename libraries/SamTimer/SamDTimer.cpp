...
SamDTimer::SamDTimer(uint8_t timernum)
{
  _timernum = timernum;
}

bool SamDTimer::tc_init()
{
...
boolean SamDTimer::PWMout(boolean pwmout, uint8_t channum, uint8_t pin)
{
...
void SamDTimer::invertWave(uint8_t invert)
{
...
void SamDTimer::setPeriodMatch(uint32_t period, uint32_t match, uint8_t channum)
{
...
void SamDTimer::setCallback(boolean enable, tc_callback cb_type, void (*callback_func)(void))
{
...
void SamDTimer::enable(boolean en)
{
  if (en)
  {
    while (tc_is_syncing(_hw));
...
void SamDTimer::timerHandler(uint8_t timerNum){
  uint32_t mask;
  switch(timerNum){
    case 3:
